...
export default function LevelPreview({children}){
    ...
    return <LinkLit to={`${BASE_URL}/select-batch?name=${children}`}>
        <h1 className="font-bold text-white big">
            {children.toUpperCase().replace('_', ' / ')}
        </h1>

        <div className="flex flex-wrap justify-center mt-6 mb-2 w-full gap-x-4 px-4">
            {words}
        </div>

        <div className="flex items-center w-full gap mt-auto">
